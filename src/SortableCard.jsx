...
const SortableCard = ({ id, label, moveCard, addCard, findCard, deleteCard }) => {
  const originalIndex = findCard(id).index

  const [{ isDragging }, drag] = useDrag({
    item: { type: 'card', id, originalIndex, cardFromSearch: false },
    collect: monitor => {
      const isItemBeingDragged = monitor.getItem() && monitor.getItem().id === id
